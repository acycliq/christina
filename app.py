...
def stack_to_images(filename):
    dapi = skimage.io.imread(filename)
    n = dapi.shape[0]
    for i in range(n):
        page = normalize_img(dapi[i])
        img = Image.fromarray(page.astype(np.uint8))
        target_file = os.path.join(os.path.join('pages', 'page_%s.jpg' % str(i).zfill(3)))
        ...
        target_file = os.path.join(os.path.join('pages', 'resized_page_%s.jpg' % str(i).zfill(3)))
